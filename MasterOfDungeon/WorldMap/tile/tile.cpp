...
Tile::Tile(std::uint16_t tile) : _tile(tile)
{
}
...
{
    _replaceGroupOfBits(ILLUMINATION_MASK, level, MAX_ILLUMINATION, ILLUMINATION_SHIFT);
}
...
void Tile::_replaceGroupOfBits(std::uint16_t targetBits, std::uint16_t newValue, std::uint16_t maxValue,
                               std::uint16_t shift)
{
    if (newValue <= maxValue)
        assert(":ERROR: maximum value is exceeded");
